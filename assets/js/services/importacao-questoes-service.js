...
function shouldCreateSharedBlock(questions, context) {
  const textoBase = sanitize(resolveSharedSupportCandidate(questions, context).textoApoio);
  return Boolean(
...
function resolveSharedSupportCandidate(questions, context = {}) {
  const contextText = sanitize(context.textoBase);
  if (contextText && Array.isArray(questions) && questions.length > 1 && questions.every(question => sanitize(question.textoApoio) === contextText)) {
    return {
      textoApoio: contextText,
      fonteTextoApoio: questions.find(question => sanitize(question.fonteTextoApoio))?.fonteTextoApoio || "",
      imagemApoio: questions.find(question => sanitize(question.imagemApoio))?.imagemApoio || "",
      fonteImagemApoio: questions.find(question => sanitize(question.fonteImagemApoio))?.fonteImagemApoio || "",
      origemDeteccao: "texto_base"
    };
  }

  const groups = new Map();
  (questions || []).forEach(question => {
    const text = sanitize(question.textoApoio);
    if (!isTextoApoioValido(text, questions)) return;
  ...
  if (!repeated) {
    return { textoApoio: "", fonteTextoApoio: "", imagemApoio: "", fonteImagemApoio: "", origemDeteccao: "" };
  }

  return {
    textoApoio: sanitize(repeated[0].textoApoio),
    fonteTextoApoio: repeated.find(question => sanitize(question.fonteTextoApoio))?.fonteTextoApoio || "",
    imagemApoio: repeated.find(question => sanitize(question.imagemApoio))?.imagemApoio || "",
    fonteImagemApoio: repeated.find(question => sanitize(question.fonteImagemApoio))?.fonteImagemApoio || "",
    origemDeteccao: "apoio_repetido"
  };
}

export function detectSharedSupportBlocks(questions, context = {}) {
  const candidate = resolveSharedSupportCandidate(questions, context);
  if (!sanitize(candidate.textoApoio) || !shouldCreateSharedBlock(questions, { ...context, textoBase: candidate.textoApoio })) {
  ...
  const blocoId = buildImportBlockId({ ...context, textoBase: candidate.textoApoio });
  ...
  return questions.map((question, index) => ({
    ...
    blocoTitulo,
    ordemBloco: index,
    blocoTextoApoio: candidate.textoApoio,
    blocoFonteTextoApoio: candidate.fonteTextoApoio || "",
    blocoImagemApoio: candidate.imagemApoio || "",
    blocoFonteImagemApoio: candidate.fonteImagemApoio || "",
    blocoOrigemDeteccao: candidate.origemDeteccao,
...
export function parseQuestoesImportadas(rawText, context) {
  ...
  const normalizedQuestions = detectSharedSupportBlocks(normalizedResults, finalContext);
  const detectedBlockText = normalizedQuestions[0]?.blocoTextoApoio || textoBase;

  return {
    tituloDetectado: titulo,
    textoBaseDetectado: detectedBlockText,
    possuiBlocoCompartilhado: Boolean(normalizedQuestions[0]?.blocoId),
